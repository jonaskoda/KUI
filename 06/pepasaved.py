...
def KNN(test_path, train_data, labels, vectors_img, k):
...
def main():
    ...
    fd = open(args.train_path+"/truth.dsv", 'r')
    for line in fd:
        tmp = line.strip().split(':')
        labels[tmp[0]] = tmp[1]
    #print(labels.values())
    ...
    if args.k is not None:
        ...
        classified = KNN(args.test_path, args.train_path, labels, vectors_img, args.k)
        fd = open(args.o, 'w')
