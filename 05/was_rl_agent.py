...
def get_best_next_reward(q_table, next_state):
    best_next_reward = 0
...
def learn_policy(env):
    ...
    while abs(start_time - time.time()) < 18:
        ...
        while not is_done and t < MAX_T:
            ...
            best_next_reward = get_best_next_reward(q_table, next_state)
            
            trial = reward + gamma * best_next_reward
            new_q_value = ((1 - alpha) * old_q_value) + alpha * (trial)
        ...
        epsilon = epsilon_decay_end + (epsilon_decay_start - epsilon_decay_end) * np.exp(-epsilon_decay_rate * episode_cnt)
